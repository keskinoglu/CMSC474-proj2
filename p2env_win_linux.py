...
def assign_player_and_compile(txtfileLine):
	if txtfileLine.split('.')[-1].strip() == 'cpp':
		...
		this_player = "timeout 1s " + "./" + txtfileLine.strip() + " < input.in > output.out"
	elif txtfileLine.split('.')[-1].strip() == 'java':
		os.system("javac " + txtfileLine.strip())
		this_player = "timeout 1s " + "java " + txtfileLine.split('.')[0].strip() + " < input.in > output.out"
	else:
		#"timeout 1s " + 
		this_player = "timeout 1s python " + txtfileLine.strip() + " < input.in > output.out"
